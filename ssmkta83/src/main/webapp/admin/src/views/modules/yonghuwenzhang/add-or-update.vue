<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="select" v-if="type!='info'" label="编辑账号" prop="bianjizhanghao">
          <el-select  @change="bianjizhanghaoChange" v-model="ruleForm.bianjizhanghao" placeholder="请选择编辑账号">
            <el-option
                v-for="(item,index) in bianjizhanghaoOptions"
                v-bind:key="index"
                :label="item"
                :value="item">
            </el-option>
        ...
        <div v-else>
          <el-form-item class="input"v-if="ruleForm.bianjizhanghao" label="编辑账号" prop="bianjizhanghao">
...
export default {
  ...
  methods: {
      ...
      this.$http({
		...
		if(json.xingming!=''&&json.xingming){
          ...
          this.$message.error(data.msg);
            ...
            this.$http({
              url: `option/bianji/bianjizhanghao`,
              method: "get"
            }).then(({ data }) => {
              if (data && data.code === 0) {
                this.bianjizhanghaoOptions = data.data;
              } else {
                this.$message.error(data.msg);
              }
            });
    ...
    // 下二随
    bianjizhanghaoChange () {
      this.$http({
        url: `follow/bianji/bianjizhanghao?columnValue=`+ this.ruleForm.bianjizhanghao,
        method: "get"
      }).then(({ data }) => {
        if (data && data.code === 0) {
              if(data.data.bianjixingming){
                this.ruleForm.bianjixingming = data.data.bianjixingming
              }
        } else {
          this.$message.error(data.msg);
        }
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `yonghuwenzhang/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.yonghuwenzhangCrossAddOrUpdateFlag = false;
                  this.parent.search();
