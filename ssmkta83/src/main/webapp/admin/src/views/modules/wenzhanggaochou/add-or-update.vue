<template>
  <div class="addEdit-block">
    ...
    >
      ...
      <el-col :span="12">
        <el-form-item class="input" v-if="type!='info'"  label="文章字数" prop="wenzhangzishu">
          <el-input v-model="ruleForm.wenzhangzishu" 
              placeholder="文章字数" clearable  :readonly="ro.wenzhangzishu"></el-input>
        </el-form-item>
        <div v-else>
          <el-form-item class="input" label="文章字数" prop="wenzhangzishu">
              <el-input v-model="ruleForm.wenzhangzishu" 
                placeholder="文章字数" readonly></el-input>
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
      ...
      <el-col :span="24">  
        <el-form-item class="upload" v-if="type!='info' && !ro.piaoju" label="票据" prop="piaoju">
          <file-upload
          tip="点击上传票据"
          action="file/upload"
          :limit="3"
        ...
        <div v-else>
          <el-form-item v-if="ruleForm.piaoju" label="票据" prop="piaoju">
            <img style="margin-right:20px;" v-bind:key="index" v-for="(item,index) in ruleForm.piaoju.split(',')" :src="item" width="100" height="100">
          </el-form-item>
        </div>
      </el-col>
      <el-col :span="12">
  ...
  methods: {
    ...
    onSubmit() {
      ...
      this.$refs["ruleForm"].validate(valid => {
        if (valid) {
          this.$http({
            url: `wenzhanggaochou/${!this.ruleForm.id ? "save" : "update"}`,
            method: "post",
            data: this.ruleForm
          }).then(({ data }) => {
            if (data && data.code === 0) {
              this.$message({
                ...
                onClose: () => {
                  ...
                  this.parent.wenzhanggaochouCrossAddOrUpdateFlag = false;
                  this.parent.search();
