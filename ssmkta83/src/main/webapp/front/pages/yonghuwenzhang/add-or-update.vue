	...
	<view class="content">
		<form class="app-update-pv">
			...
			<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group select">
				...
				<picker @change="bianjizhanghaoChange" :value="bianjizhanghaoIndex" :range="bianjizhanghaoOptions">
					<view :style='{"boxShadow":"0 0 4rpx rgba(0,0,0,.6) inset","backgroundColor":"rgba(255, 255, 255, 0)","borderColor":"rgba(0,0,0,.6)","borderRadius":"16rpx","color":"rgba(23, 152, 242, 1)","textAlign":"left","borderWidth":"2rpx","fontSize":"28rpx","borderStyle":"solid","height":"88rpx"}' class="uni-input">{{bianjizhanghaoOptions[bianjizhanghaoIndex]}}</view>
				</picker>
			</view>
			<view :style='{"boxShadow":"0 0 16rpx rgba(0,0,0,.3)","backgroundColor":"rgba(255, 255, 255, 0.25)","borderColor":"#ccc","margin":"0 0 20rpx 0","borderWidth":"0","borderStyle":"solid","height":"108rpx"}' class="cu-form-group">
	...
	export default {
		data() {
			return {
				...
				bianjizhanghaoOptions: [],
				bianjizhanghaoIndex: 0,
				// 登陆用户信息
				user: {},
                                ro:{
                                   biaoti : false,
		...
		async onLoad(options) {
			...
			// 下2
			res = await this.$api.option(`bianji`,`bianjizhanghao`,{});
			this.bianjizhanghaoOptions = res.data;
			...
			if (options.id) {
				...
				// 获取信息
				res = await this.$api.info(`yonghuwenzhang`, this.ruleForm.id);
		...
		methods: {
			...
			// 下二随
			async bianjizhanghaoChange (e) {
				this.bianjizhanghaoIndex = e.target.value
				this.ruleForm.bianjizhanghao = this.bianjizhanghaoOptions[this.bianjizhanghaoIndex]
				let res = await this.$api.follow(`bianji`, `bianjizhanghao`,{
					columnValue: this.ruleForm.bianjizhanghao
				});
				if(res.data.bianjixingming){
					this.ruleForm.bianjixingming = res.data.bianjixingming
				}
			},
			...
			async onSubmitTap() {
				if(this.ruleForm.id){
					await this.$api.update(`yonghuwenzhang`, this.ruleForm);
				}else{
					await this.$api.add(`yonghuwenzhang`, this.ruleForm);
				}
