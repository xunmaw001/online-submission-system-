		...
		<view class="container">
			...
			<view class="bottom-content cu-bar bg-white tabbar border shop">
				...
				<view style="text-align: right;">
					<button v-if="isAuth('yonghuwenzhang','文章推送')" @tap="onAcrossTap('wenzhangzhanshi')" class="cu-btn bg-brown round shadow-blur" style="margin-right: 10upx;min-width: 180upx;" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[0+6],borderColor:btnColor[0+6]}">文章推送</button>
					<button v-if="isAuth('yonghuwenzhang','审核')" @tap="onSHTap" style="margin-right: 10upx;" class="cu-btn bg-red round shadow-blur" :style="{width:'auto',borderRadius:'8rpx',height:'80rpx',fontSize:'28rpx',color:'#fff',backgroundColor:btnColor[4],borderColor:btnColor[4]}">审核</button>
                </view>
			</view>
		</view>
...
<script>
	export default {
		...
		methods: {
			// 支付
			onPayTap(){
				uni.setStorageSync('paytable','yonghuwenzhang');
			...
			onAcrossTap(tableName){
     			uni.setStorageSync('crossTable','yonghuwenzhang');
				uni.setStorageSync(`crossObj`, this.detail);
				this.$utils.jump(`../${tableName}/add-or-update?cross=true`);
			},
			// 获取详情
			async init(){
				let res = await this.$api.info('yonghuwenzhang', this.id);
			...
			// 添加评论
			onCommentTap() {
				this.$utils.jump(`../discussyonghuwenzhang/add-or-update?refid=${this.id}`)
			},
			// 点赞
			async zan() {
				let _this = this;
				uni.showModal({
					...
					success: async function(res) {
						if (res.confirm) {
							...
							await _this.$api.update('yonghuwenzhang', _this.detail);
			...
			async onFinishTap() {
				...
				await this.$api.update('yonghuwenzhang', this.detail);
				this.$utils.msg('审核成功');
